...
class InterpolateData(BaseData):
    def _preprocess_data(self, elm_indices: np.ndarray = None, shuffle_sample_indices: bool = False, ) -> Tuple[
        ...
        # )
        signals = self._interpolate_signals(signals)
    ...
    def _interpolate_signals(self, signals) -> np.ndarray:
        signals_resized = []
        for signal in signals:
            signal = cv2.resize(signal, dsize=(self.args.interpolate_size, self.args.interpolate_size),
                    interpolation=cv2.INTER_NEAREST, )
            signals_resized.append(signal)

        return np.array(signals_resized)
...
#     )
#     data = InterpolateData(args, logger)
