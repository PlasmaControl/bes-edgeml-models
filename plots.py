...
class MyModel(nn.Module):
    ...
    dataset = datasets.MNIST(root='PATH', transform=transforms.ToTensor(), download=True)
    loader = DataLoader(dataset, num_workers=2, batch_size=8, shuffle=True)
...
def plot_voxels(n_voxels):
    ...
    fig = plt.figure()
    ax = fig.add_subplot(projection='3d', xlim=n_voxels.shape[0], ylim=n_voxels.shape[1], zlim=n_voxels.shape[2])
