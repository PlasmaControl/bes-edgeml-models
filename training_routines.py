...
def show_mse_loss(use_saved=True, superimposed=True):
    ...
    if superimposed:
        ax1.plot(epochs,
                 ...
                 label=f'{args.vae_beta} * $D_{{kl}}$', c='r')
        ax1.plot(epochs, train_mseloss / train_mseloss.max() - (
                    train_mseloss.min() / train_mseloss.max() - train_loss.min() / train_norm), label='MSE Loss', c='b')
        ax2.plot(epochs, val_klloss / val_norm - (val_klloss.min() / val_klloss.max() - val_loss.min() / val_norm),
                 c='r')
        ax2.plot(epochs,
