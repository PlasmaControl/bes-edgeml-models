...
def get_elm_df(args: argparse.Namespace, data: tuple, ) -> Tuple[pd.DataFrame, np.ndarray, np.ndarray]:
...
def pool_features(args: argparse.Namespace, df: pd.DataFrame, signals: np.ndarray,
    ...
    for idx in range(len(df)):
        ...
        label = labels[elm_idx + args.signal_window_size + args.label_look_ahead - 1].astype("int")
        maxpool = nn.MaxPool3d(kernel_size=(args.signal_window_size, 5, 5), stride=(1, 1, 1), )
        avgpool = nn.AvgPool3d(kernel_size=(args.signal_window_size, 5, 5), stride=(1, 1, 1), )
        y_max = maxpool(torch.tensor(signal_window).view(1, 1, args.signal_window_size, 8, 8))
        y_max = y_max.view(-1)
        y_avg = avgpool(torch.tensor(signal_window).view(1, 1, args.signal_window_size, 8, 8))
        y_avg = y_avg.view(-1)
    ...
    expanded_max_df = pd.DataFrame(df["max_pool_features"].to_list(),
            columns=[f"max_pool_{i + 1}" for i in range(len(features_max[0]))], )
    expanded_avg_df = pd.DataFrame(df["avg_pool_features"].to_list(),
...
if __name__ == "__main__":
    ...
    if not args.dry_run:
        train_df.to_csv(os.path.join(roc_dir, f"train_features_df_{args.label_look_ahead}.csv"), index=False, )
        valid_df.to_csv(os.path.join(roc_dir, f"valid_features_df_{args.label_look_ahead}.csv"), index=False, )
    print("-" * 20)
