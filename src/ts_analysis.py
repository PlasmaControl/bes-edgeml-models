...
def plot_autocorr(args: argparse.Namespace, column: str, df: pd.DataFrame):
    ...
    if not args.dry_run:
        plt.savefig(os.path.join(args.output_dir, f"auto_correlation_plots_elm_id_46_{column}.png", ), dpi=150, )
    plt.show()


def decompose_signal(args: argparse.Namespace, signal_channel: str, df: pd.DataFrame):
...
if __name__ == "__main__":
    ...
    signal_df = pd.DataFrame(signal.tolist(), columns=[f"ch:{i}" for i in range(1, 65)])
    print(signal_df)
