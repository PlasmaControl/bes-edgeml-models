...
class Data:
    def __init__(self, args: argparse.ArgumentParser, logger: logging.getLogger, datafile: str = None,
            normalize: bool = False, truncate_inputs: bool = False, ):
        ...
        self.logger.info(f"Total frames in the whole data: {self._get_total_frames()}")  # self.transition = np.linspace(  #     0, 1, 2 * self.args.transition_halfwidth + 3  # )

    def get_data(self, shuffle_sample_indices: bool = False, fold: int = None) -> Tuple:
        ...
        self.logger.info("-" * 30)
        test_data = self._preprocess_data(test_elms, shuffle_sample_indices=shuffle_sample_indices, is_test_data=True, )
    ...
    def _preprocess_data(self, elm_indices: np.ndarray = None, shuffle_sample_indices: bool = False,
            is_test_data: bool = False, ) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        for elm_index in elm_indices:
            ...
            indices_data = self._get_valid_indices(_signals=_signals,
                    _labels=_labels,
        ...
        sample_indices = self._oversample_data(_labels, valid_indices, elm_start, elm_stop)
    ...
    def _partition_elms(self, max_elms: int = None, fold: int = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        if self.kfold and fold is None:
            raise ValueError(f"K-fold cross validation is passed but fold index in range [0, {self.args.folds}) is not specified.")
    ...
    def _kfold_cross_val(self, training_elms: np.ndarray) -> None:
        ...
        """
        kf = model_selection.KFold(n_splits=self.args.folds, shuffle=True, random_state=self.args.seed)
    ...
    def _oversample_data(self, _labels: np.ndarray, valid_indices: np.ndarray, elm_start: np.ndarray,
            elm_stop: np.ndarray, index_buffer: int = 20, ) -> np.ndarray:
        ...
        self.logger.info(f"Active ELM oversampling for balanced data: {oversample_count}")
        if self.data_mode == "balanced":
            for i_start, i_stop in zip(elm_start, elm_stop):
                assert np.all(_labels[i_start: i_stop + 1] >= 0.5)
                active_elm_window = np.arange(i_start - index_buffer, i_stop + index_buffer, dtype="int")
            ...
            fraction_elm = (np.count_nonzero(_labels[sample_indices] >= 0.5) / sample_indices.size)
            self.logger.info(f"Active ELM fraction (balanced data): {fraction_elm:.3f}")
        return sample_indices


class ELMDataset(torch.utils.data.Dataset):
    def __init__(self, args: argparse.ArgumentParser, signals: np.ndarray, labels: np.ndarray,
            sample_indices: np.ndarray, window_start: np.ndarray, logger: logging.getLogger,
    ...
    def __getitem__(self, idx: int):
        ...
        signal_window = self.signals[elm_idx: elm_idx + self.args.signal_window_size]
        label = self.labels[elm_idx + self.args.signal_window_size + self.args.label_look_ahead - 1].astype("int")
        if self.args.stack_elm_events:
            if self.args.signal_window_size == 8:
                signal_window = np.hsplit(np.concatenate(signal_window, axis=-1), 2)
                signal_window = np.concatenate(signal_window)
            elif self.args.signal_window_size == 16:
                signal_window = np.hsplit(np.concatenate(signal_window, axis=-1), 4)
                signal_window = np.concatenate(signal_window)
            else:
                raise Exception(f"Expected signal window size is 8 or 16 but got {self.args.signal_window_size}.")
            if self.transform:
                signal_window = self.transform(image=signal_window)["image"]
        if self.args.add_noise:
            noise = np.random.normal(loc=self.args.mu, scale=self.args.sigma, size=signal_window.shape, )
            signal_window += noise
        ...
        try:
            if self.args.data_preproc == 'interpolate':
                interp_size = (self.args.interpolate_size, self.args.interpolate_size,)
                signal_window = torch.nn.functional.interpolate(signal_window, size=interp_size)
...
if __name__ == "__main__":
    ...
    logger.info(f"Window start indices - shape: {window_start.shape}, first five: {window_start[:5]}, last five: {window_start[-5:]}")
    transforms = get_transforms(args)

    train_dataset = ELMDataset(args, *train_data, logger=logger, transform=transforms, )
