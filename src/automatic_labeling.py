...
if __name__ == "__main__":
    ...
    for i_elm in range(num_elms):
        print(f"{i_elm}. Processing elm event with start index: {window_start[i_elm]}")
        ...
        if args.plot_data and i_elm == 0:
            ...
            for ax in axs:
                for i, a in zip(range(4), [0.9, 0.8, 0.75, 0.7]):
                    ax.plot(time_grad[:, i + hop], label=f"ch: {i + hop + 1}", alpha=a)
                ax.plot(label[::4], c="k", linestyle="--", label="ground truth", alpha=0.7, )
                ax.legend(fontsize=7, frameon=False)
                ax.spines["left"].set_color("gray")
                ax.spines["bottom"].set_color("gray")
                hop += 4
            plt.suptitle(f"Time derivatives, hop-length: 4, signal window size: {args.signal_window_size}",
                    fontsize=18, )
            ...
            for ax in axs:
                for i, a in zip(range(4), [0.9, 0.8, 0.75, 0.7]):
                    ax.plot(time_grad_diffs[:, i + hop], label=f"ch: {i + hop + 1}", alpha=a, )
                ax.plot(label[::4], c="k", linestyle="--", label="ground truth", alpha=0.7, )
                ax.legend(fontsize=7, frameon=False)
                ax.spines["left"].set_color("gray")
                ax.spines["bottom"].set_color("gray")
                hop += 4
            plt.suptitle(f"Time derivatives' differences, hop-length: 4, signal window size: {args.signal_window_size}",
                    fontsize=18, )
            plt.tight_layout(rect=[0, 0.03, 1, 0.95])
            if not args.dry_run:
                plt.savefig(os.path.join(output_dir,
        ...
        channels = [col for col in df.columns if col not in ["elm_event_index", "manual_label"]]
        ...
        if not x:
            print(f"Found no active elm with serial no: {i_elm} and start index: {window_start[i_elm]} ")
    ...
    dfs.loc[:, ["elm_id", "elm_event", "automatic_label"]].to_csv(
            os.path.join(roc_dir, f"automatic_labels_df_sws_{args.signal_window_size}_{args.label_look_ahead}.csv", ),
            index=False, )
    ...
    if args.plot_data:
        ...
        elm_event_list = [elm_index_list[i * 12: (i + 1) * 12] for i in range(num_pages)]
        for page_num, page in enumerate(elm_event_list):
            ...
            for i in range(len(page)):
                ...
                ax.plot(signals_list[i + (12 * page_num)][:, 0, 0] / 10.0, lw=1.0, label="ch: 1", alpha=0.6, )
                ax.plot(signals_list[i + (12 * page_num)][:, 2, 6] / 10.0, lw=1.0, label="ch: 22", alpha=0.6, )
                ax.plot(signals_list[i + (12 * page_num)][:, 7, 7] / 5.0, lw=1.0, label="ch: 64", alpha=0.6, )
                ax.set_title(f"ELM index: {all_elms[i + (12 * page_num)]:05d}", fontsize=12, )
            ...
            if not args.dry_run:
                fname = os.path.join(output_dir, f"page_{page_num + 1}_elm_{page[0] + 1}_{page[-1] + 1}.png", )
