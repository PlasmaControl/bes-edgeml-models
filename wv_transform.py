...
if __name__ == "__main__":
    ...
    channels = [0, 31, 63]
    wavelet_fam = {"db": ["db2", "db4", "db10", "db20"], "sym": ["sym2", "sym9", "sym14", "sym18"],
            "bior": ["bior2.8", "bior3.1", "bior3.9", "bior5.5"], "coif": ["coif1", "coif4", "coif10", "coif17"], }
    ...
    for wv_fam in list(wavelet_fam.keys()):
        ...
        for wavelet in wavelet_fam[wv_fam]:
            ...
            coeff = pywt.wavedec(signal, wavelet=wavelet, mode="symmetric", axis=0)
            uthresh = 1
            coeff[1:] = (pywt.threshold(i, value=uthresh, mode="hard") for i in coeff[1:])
            reconst_signal = pywt.waverec(coeff, wavelet=wavelet, mode="symmetric", axis=0)
        ...
        for ch in channels:
            ...
            for (k, v), off in zip(reconst_signals.items(), offsets):
                w = pywt.Wavelet(k)
                ax.plot(v[1900:2300, ch] + off, label=f"{k}, length: {w.dec_len}", lw=1.5, )
            ax.plot(signal[1900:2300, ch] + offsets[-1], label="original", lw=1.0, c="slategray", )
