...
class VAEfcls32Model(nn.Module):

    def __init__(self, args: argparse.Namespace, num_filters: int = 256,
                 fc_units: Union[int, Tuple[int, int]] = (128, 64), latent_dim: int = 32, beta: float = 1.0,
                 dropout_rate: float = 0.4, negative_slope: float = 0.02, maxpool_size: int = 2, ):
        """
        ...
        super(VAEfcls32Model, self).__init__()
        self.args = args
        self.latent_dim = latent_dim
        self.beta = beta
        self.logscale = nn.Parameter(torch.Tensor([0.0]))
        self.in_size = self.args.signal_window_size * 8 * 8

        self.encoder = nn.Sequential(OrderedDict([('flatten', nn.Flatten()), (
        'fc0', nn.Linear(in_features=self.in_size, out_features=num_filters)), ('dropout0', nn.Dropout(p=dropout_rate)),
                                                  ('relu0', nn.LeakyReLU(negative_slope=negative_slope)),
                                                  ...
                                                  ('relu2', nn.LeakyReLU(negative_slope=negative_slope)), ('fc3',
                                                                                                           nn.Linear(
                                                                                                               in_features=
                                                                                                               fc_units[
                                                                                                                   1],
