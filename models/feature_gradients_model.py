...
class SpatialFeatures(nn.Module):
    def __init__(self, args: argparse.Namespace, num_filters: int = 10, maxpool_size: int = 2, ):
        ...
        self.conv_spatial = nn.Conv3d(in_channels=1, out_channels=num_filters, kernel_size=self.filter_size, )
...
class TemporalFeatures(nn.Module):
    def __init__(self):
        ...
        self.conv_temporal = nn.Conv2d(in_channels=1, out_channels=32, kernel_size=5)
...
class FeatureGradientsModel(nn.Module):
    def __init__(self, args: argparse.Namespace, spatial: SpatialFeatures, temporal: TemporalFeatures,
            dropout_rate: float = 0.4, negative_slope: float = 0.02, ):
        ...
        #     input_features = 5050
            ...
            raise ValueError("Input features for given signal window size are not parsed!")
...
if __name__ == "__main__":
    ...
    args = parser.parse_args(["--signal_window_size", "16", ],  # ["--device", "cpu"]
    )
    shape = (4, 1, 16, 8, 8)
    x = torch.rand(*shape)
