...
def predict(args: argparse.Namespace, test_data: tuple, model: object, hook_layer: str, device: torch.device) -> dict:
    ...
    for i_elm in range(num_elms):
        ...
        predictions = np.zeros(elm_labels.size - args.signal_window_size - args.label_look_ahead + 1)
        for j in range(predictions.size):
            if args.use_gradients:
                input_signals = np.array(elm_signals[j: j + args.signal_window_size, :, :].reshape([1,
                                                                                                    args.signal_window_size,
                                                                                                    8, 8, 6]),
                        dtype=np.float32, )
                input_signals = np.transpose(input_signals, axes=(0, 4, 1, 2, 3))
            else:
                input_signals = np.array(elm_signals[j: j + args.signal_window_size, :, :].reshape([1, 1,
                                                                                                    args.signal_window_size,
                                                                                                    8, 8]),
                        dtype=np.float32, )
        ...
        macro_predictions_active_elms = np.array([np.any(micro_predictions_active_elms).astype(int)])
        ...
        elm_predictions[window_start[i_elm]] = {"activations": np.asarray(activations_), "weights": weights,
                                                "signals": elm_signals, "labels": elm_labels,
                                                "micro_predictions": micro_predictions, "macro_labels": macro_labels,
                                                "macro_predictions": macro_predictions, "elm_time": elm_time, }
    return elm_predictions


def plot_boxes(elm_predictions: dict, layer: str = None):
...
def plot(args: argparse.Namespace, elm_predictions: dict, plot_dir: str, elms: List[int], elm_range: str,
    ...
    for i, i_elm in enumerate(elms):
        ...
        if flag:
            plt.axvline(elm_start - args.truncate_buffer,
                        ymin=0,
                        ymax=0.9,
                        c="r",
                        ls="--",
...
def show_metrics(args: argparse.Namespace, y_true: np.ndarray, y_probas: np.ndarray, report_dir: str, roc_dir: str,
    ...
    if pred_mode == "micro":
        if np.array_equal(y_probas, y_probas.astype(bool)):
            raise ValueError("Metrics for micro mode require micro_predictions but macro_predictions are passed.")
        ...
        ax.text(x=0.5, y=1.05, s="Micro predictions", fontsize=18, ha="center", va="bottom", transform=ax.transAxes, )
        ax.text(x=0.5,
                y=1.01,
        ...
        if not np.array_equal(y_probas, y_probas.astype(bool)):
            raise ValueError("Metrics for macro mode require macro_predictions but micro_predictions are passed.")
        ...
        ax.set_xlabel("Predicted Label", fontsize=14)
        ax.set_ylabel("True Label", fontsize=14)
        ax.text(x=0.5, y=1.05, s="Macro predictions", fontsize=18, ha="center", va="bottom", transform=ax.transAxes, )
        ax.text(x=0.5,
                y=1.01,
                s=f"Signal window: {args.signal_window_size}, Label look ahead: {args.label_look_ahead}",
                fontsize=12,
        ...
        raise ValueError(f"Expected pred_mode to be either `micro` or`macro` but {pred_mode} is passed.")
...
def model_predict(model, device: torch.device, data_loader: torch.utils.data.DataLoader, ) -> None:
    ...
    print(f"ROC score on test data: {metrics.roc_auc_score(targets, predictions)}")
...
def main(args: argparse.Namespace, ) -> None:
    ...
    (test_data_dir, model_ckpt_dir, clf_report_dir, plot_dir, roc_dir,) = output_paths
    ...
    test_data, test_dataset = get_test_dataset(args, file_name=test_fname, logger=logger)
    ...
    if args.show_metrics:
        ...
        show_metrics(args, targets_micro, predictions_micro, clf_report_dir, roc_dir, plot_dir, pred_mode="micro", )
        # show metrics for macro predictions
        targets_macro, predictions_macro = get_dict_values(pred_dict, mode="macro")
        show_metrics(args, targets_macro, predictions_macro, clf_report_dir, roc_dir, plot_dir, pred_mode="macro", )
